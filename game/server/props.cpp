...
void CBreakableProp::Break( CBaseEntity *pBreaker, const CTakeDamageInfo &info )
{
	...
	if ( gpGlobals->maxClients > 1 && breakable_multiplayer.GetBool() )
	{
		...
		case MULTIPLAYER_BREAK_CLIENTSIDE:
			te->PhysicsProp( filter, -1, GetModelIndex(), m_nSkin, GetAbsOrigin(), GetAbsAngles(), velocity, true, GetEffects(), GetRenderColor24() );
			break;
	...
	if ( m_damageStates.Count() )
	{
		...
		if ( targetDamageState > m_currentDamageState )
		{
			...
			while ( targetDamageState > m_currentDamageState )
			{
				...
				te->PhysicsProp( filter, -1, GetModelIndex(), m_nSkin, GetAbsOrigin() + offset, GetAbsAngles(), force, 1, GetEffects(), GetRenderColor24() );
	...
	if ( oldHealth != newHealth && newHealth > 0 && m_damageStates.Count() > 0 )
	{
		...
		if ( targetDamageState > m_currentDamageState )
		{
			...
			while ( targetDamageState > m_currentDamageState )
			{
				...
				te->PhysicsProp( filter, -1, GetModelIndex(), m_nSkin, GetAbsOrigin(), GetAbsAngles(), addedVelocity, 1, GetEffects(), GetRenderColor24() );
