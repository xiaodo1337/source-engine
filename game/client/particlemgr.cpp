...
bool CParticleMgr::EarlyRetireParticleSystems( int nCount, CNewParticleEffect **ppEffects )
{
	...
	render->GetMatricesForView( *pViewSetup, &dummy1, &dummy2, &dummy3, &worldToScreen );
	flFocalDist = tan( DEG2RAD( pViewSetup->fov * 0.5f ) );
...
// #endif
	...
	if ( nCount )
	{
		...
		else
		{
			...
			else
			{
				...
				processor.Run( particlesToSimulate.Base(), nCount, 1, m_pThreadPool[nAltCore-1] );
			}
		}
	}
	...
	{
		ParallelProcess( "CParticleMgr::UpdateNewEffects", nonDrawingSimulateList.Base(), nonDrawingSimulateList.Count(), ProcessNonDrawingSystem, PreProcessPSystem, PostProcessPSystem );
	}
