	...
	const char *pDebugName )
{
	...
	return CreateOrAggregate( pOwner, pDef, vecAggregatePosition, pDebugName );
}
...
int CNewParticleEffect::DrawModel( int flags )
{
	...
	{
		C_BasePlayer *pCameraObject = CBasePlayer::GetLocalPlayer();
		...
		Render( nViewRecursionLevel, pRenderContext, vecDiffuseModulation, ( flags & STUDIO_TRANSPARENCY ) ? IsParticleSystemTwoPass() : false, pCameraObject );
		pRenderContext->MatrixMode( MATERIAL_MODEL );
	...
	if ( cl_particles_show_controlpoints.GetBool() )
	{
		for ( int i = 0; i < MAX_PARTICLE_CONTROL_POINTS; ++i )
		{
			if ( ReadsControlPoint( i ) )
			{
				...
				Vector vecForward, vecRight, vecUp;
				GetControlPointOrientationAtCurrentTime( i, &vecForward, &vecRight, &vecUp );
				debugoverlay->AddLineOverlay( vecPos, vecPos + ( vecForward * 4.0f ), 255, 0, 0, true, 0.05f );
				debugoverlay->AddLineOverlay( vecPos, vecPos + ( vecRight * 4.0f ), 0, 255, 0, true, 0.05f );
				debugoverlay->AddLineOverlay( vecPos, vecPos + ( vecUp * 4.0f ), 0, 0, 255, true, 0.05f );
			}
		}
	}
